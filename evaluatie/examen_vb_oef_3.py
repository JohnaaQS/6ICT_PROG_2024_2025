...
"""
totprijs = 0
for index, prijs in enumerate(producten):
    prijs = producten[index]["prijs"]
    voorraad = producten[index]["voorraad"]
    stockwaarde = prijs * voorraad
    print(f"{producten[index]["naam"]} heeft een totaalwaarde van {stockwaarde}")
    totprijs = totprijs + stockwaarde
print(f"de totale prijs is {totprijs}")
...
"""
vraag = int(input("Zoek producten met minder voorraad dan: "))
for product in producten:
    if product["voorraad"] < vraag:
        print(f"{product["naam"]} heeft nog maar {product["voorraad"]} stuks op voorraad")
...
"""
vraag = input("geef product: ")
for product in producten:
    if product["naam"] == vraag:
        print(f"{product["naam"]} is aanwezig")
    else:
        print("niet aanwezig")
        add = input()
