...
for shares, aandeel in aandelen.items():
    print(f"- {aandeel} shares in {shares}")
...
while True: 
 Naam = input("geef de naam op van het aandeel: ")
 if Naam == "stop":
     break
 value = aandelen[Naam]
 print(f"aandeel: {Naam}, met waarde {value}")
 if Naam in aandelen:
     ver = input("wil je kopen of verkopen kopen is 'k' en verkopen is 'v' ")
     if ver == "v": 
      stuks = int(input("hoeveel stuks wil je verkopen "))
      waarde = value - stuks
      if waarde < 0: 
       print("dit kan niet")
      else: 
         waarde = value - stuks
         print(waarde)
         aandelen[Naam] = waarde
     if ver == "k": 
      stuks = int(input("hoeveel stuks wil je kopen "))
      waarde = value + stuks
      print(waarde)
